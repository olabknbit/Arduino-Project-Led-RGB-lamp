...
#define NUM_RGB       (2)         // Number of WS281X we have connected
#define NUM_BYTES     (NUM_RGB*3) // Number of LEDs (3 per each WS281X)
#define DIGITAL_PIN   (6)         // Digital port number
...
void measure_temperature() {
  // Here will be our temperature measurement
...
void render(void) 
{
  ...
  volatile uint8_t  *p    = rgb_arr;   // Copy the start address of our data array
  volatile uint8_t val  = *p++;      // Get the current byte value & point to next byte
  volatile uint8_t high = PORT |  _BV(PORT_PIN); // Bitmask for sending HIGH to pin
  volatile uint8_t low  = PORT & ~_BV(PORT_PIN); // Bitmask for sending LOW to pin
  volatile uint8_t tmp  = low;       // Swap variable to adjust duty cycle 
  volatile uint8_t nbits= NUM_BITS;  // Bit counter for inner loop
